...
class dataset(Dataset):
    def __init__(self, mode = "train", num_batch=30, n_way=1, k_shot=5, k_query=10):
        ...
        if mode == "train":
            self.input_data = torch.tensor(pd.read_csv('../ValidityCheck_train_x.csv').values, dtype=torch.float32).view(-1, *input_shape)
            self.labels = torch.tensor(pd.read_csv('../ValidityCheck_train_y.csv').values, dtype=torch.float32)
        else:
            self.input_data = torch.tensor(pd.read_csv('../ValidityCheck_test_x.csv').values, dtype=torch.float32).view(-1, *input_shape)
            self.labels = torch.tensor(pd.read_csv('../ValidityCheck_test_y.csv').values, dtype=torch.float32)
